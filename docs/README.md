# Docs

| File | What it is |
|------|------------|
| [../README.md](../README.md) | English landing page and quick start |
| [../README.zh-CN.md](../README.zh-CN.md) | 简体中文 |
| [architecture.md](architecture.md) | How the three scanners merge, and how a hub pulls peer maps |
| [deployment.md](deployment.md) | Image tags, socket proxy, reverse proxy, NAS notes |
| [roadmap.md](roadmap.md) | Upcoming work and out of scope |
| [../SECURITY.md](../SECURITY.md) | Trust model and how to report issues |
| [../CONTRIBUTING.md](../CONTRIBUTING.md) | Dev loop and pull requests |
| [../CHANGELOG.md](../CHANGELOG.md) | Release notes |

Screenshots used in the README: [screenshot.png](screenshot.png), [screenshot-multi-host.png](screenshot-multi-host.png).
