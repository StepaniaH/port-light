...
Rules apply when explicitly selected for allocation. They do not reserve the whole range, bind sockets, change service configurations, or restrict allocations that omit a rule. **Manage ports → Find free ports** uses the selected numeric range. Settings read-only mode prevents rule edits.
